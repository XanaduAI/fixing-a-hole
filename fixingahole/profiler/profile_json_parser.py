...
class ProfileParser:
    ...
    def parse_file(self, file_path: str) -> list[FunctionProfile]:
        ...
        # Choose parsing strategy based on file extension / content
        p = Path(file_path)
        text = p.read_text(encoding="utf-8")
        # Try JSON first (modern scalene JSON output or other tool JSON)
        try:
        ...
        if parsed and isinstance(parsed, dict):
            return self.parse_json_content(parsed)

        return self.parse_content(text)

    def parse_json_content(self, content: dict) -> list[FunctionProfile]:
        """Parse profile results provided as a JSON dictionary.

        The JSON layout expected is similar to the attached `profile_results.json` where
        top-level key `files` maps file paths to file summaries with optional `functions`
        lists and a `percent_cpu_time` overall for the file. This method extracts
        function-level entries when present; otherwise it synthesizes an entry per file
        using the file-level percentage.
        """
        functions: list[FunctionProfile] = []

        # Extract walltime if available
        if self.walltime is None:
            # common key in the provided example: elapsed_time_sec
            elapsed = content.get("elapsed_time_sec")
            if isinstance(elapsed, (int, float)):
                self.walltime = float(elapsed)

        # Extract max memory if available
        if self.max_memory is None:
            # Some JSON outputs may include max memory under different keys
        ...
        for file_path, info in files.items():
            # If there are explicit functions recorded in JSON, use them
            funcs = info.get("functions") if isinstance(info, dict) else None
            if funcs:
                for fn in funcs:
                    # Try to extract fields; fn may be a dict or a string
                    if isinstance(fn, dict):
                        name = fn.get("name") or fn.get("function") or fn.get("function_name") or fn.get("label")
                        pct = fn.get("percent_cpu_time") or fn.get("percent_time") or 0
                        kwargs = {
                            "line_number": int(fn.get("line", 0) or 0),
                            "function_name": name or "<unknown>",
                            "file_path": file_path,
                            "cpu_percentages": [str(pct)],
                        }
                ...
                # No per-function info; synthesize one entry for the file using percent_cpu_time
                pct = 0
                if isinstance(info, dict):
                    pct = info.get("percent_cpu_time") or info.get("percent_time") or 0
                kwargs = {
                    "line_number": 0,
                    "function_name": Path(file_path).name,
                    "file_path": file_path,
                    "cpu_percentages": [str(pct)],
                }
                functions.append(FunctionProfile(**kwargs))
    ...
    def parse_content(self, content: str) -> list[FunctionProfile]:
        ...
        # If this is JSON-looking content, try to parse it and delegate to JSON parser
