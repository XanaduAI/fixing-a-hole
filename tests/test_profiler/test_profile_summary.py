...
class TestProfileSummaryExtraction:
    ...
    def test_generate_summary(self, advanced_profile_json: Path):
        """Test generating a summary from the profile."""
        parser = ProfileSummary(filename=advanced_profile_json)
        ...
        assert "Functions by Total Runtime" in summary
    ...
    def test_nonexistent_file(self, tmp_path: Path):
        ...
        parser = ProfileSummary(filename=nonexistent)

        # Should handle gracefully with None values
        assert parser.walltime is None
        assert parser.max_memory is None
        assert len(parser.data.functions) == 0
